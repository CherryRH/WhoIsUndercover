...
const tag: string = 'TestTag [PreferenceUtil]:'
...
class PreferenceUtil {
  ...
  loadPreference_async(context, name: string) {
    ...
    preferences.getPreferences(context, name)
      .then(pref => {
        ...
        console.info(tag, 'Preference ' + name + ' 加载成功')
        return true;
      })
      .catch(Error => {
        console.error(tag, 'Preference ' + name + ' 加载失败', JSON.stringify(Error))
        return false;
      })
  }
  ...
  async putPereferenceValue(name: string, key: string, value: preferences.ValueType) {
    if(!this.prefMap.has(name)) {
      console.error(tag, 'Preference ' + name + ' 未加载')
      return false;
    }
    try {
      ...
      //刷盘
      await pref.flush()
      console.info(tag, 'Preference ' + name + ' 保存 ' + key + ' = ' + value + ' 成功')
      return true;
    }
    catch (Error) {
      console.error(tag, 'Preference ' + name + ' 保存 ' + key + ' = ' + value + ' 失败', JSON.stringify(Error))
      return false;
    }
  }
  ...
  async getPereferenceValue(name: string, key: string, defaultValue: preferences.ValueType) {
    ...
    try {
      ...
      console.info(tag, 'Preference ' + name + ' 读取 ' + key + ' = ' + value + ' 成功')
      return value
    }
    catch (Error) {
      console.error(tag, 'Preference ' + name + ' 读取 ' + key + ' 的值失败', JSON.stringify(Error))
      return defaultValue;
    }
  }
